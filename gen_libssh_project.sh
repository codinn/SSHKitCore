...
rm -f CMakeCache.txt
cmake -Wno-dev -DCMAKE_PREFIX_PATH=${BUILD_DIR}/${OPENSSL_VERSION}-osx -DOPENSSL_ROOT_DIR= -DCMAKE_INSTALL_PREFIX=./dist -DCMAKE_BUILD_TYPE=MinSizeRel -DWITH_SSH1=OFF -DWITH_STATIC_LIB=ON -DWITH_SERVER=ON -DCMAKE_OSX_SYSROOT=${OSX_SDK} -DCMAKE_OSX_DEPLOYMENT_TARGET=10.9 -DCMAKE_OSX_ARCHITECTURES=x86_64 -GXcode ../
popd
